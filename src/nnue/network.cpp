...
template<typename Arch, typename Transformer>
Judas::Eval::NNUE::NetworkOutput Judas::Eval::NNUE::Network<Arch, Transformer>::evaluate(
    const Judas::Position& pos, 
    Judas::Eval::NNUE::AccumulatorCaches::Cache<Judas::Eval::NNUE::Network<Arch, Transformer>::FTDimensions>* cache) const {
...
template<typename Arch, typename Transformer>
void Judas::Eval::NNUE::Network<Arch, Transformer>::verify(std::string evalfilePath) const {
    if (evalfilePath.empty())
        evalfilePath = evalFile.defaultName;
